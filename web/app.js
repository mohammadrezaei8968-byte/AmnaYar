...
async function register(e){e.preventDefault();const btn=e.submitter;if(btn){btn.disabled=true;btn.textContent='در حال ثبت‌نام...'}try{const r=await api('/api/auth/register','POST',{email:$('#email').value,username:$('#username').value,password:$('#password').value});if(r.error)return alert(r.error);if(!r.user)return alert('ثبت‌نام انجام نشد؛ پاسخ نامعتبر از سرور دریافت شد.');location.href='/dashboard.html'}catch(err){alert('ارتباط با سرور برقرار نشد. لطفاً دوباره تلاش کنید.')}finally{if(btn){btn.disabled=false;btn.textContent='ثبت‌نام رایگان'}}}
async function login(e){e.preventDefault();const btn=e.submitter;if(btn){btn.disabled=true;btn.textContent='در حال ورود...'}try{const r=await api('/api/auth/login','POST',{identifier:$('#identifier').value,password:$('#password').value});if(r.error)return alert(r.error);if(!r.user)return alert('ورود انجام نشد؛ پاسخ نامعتبر از سرور دریافت شد.');location.href=r.user.role==='owner'?'/owner':'/dashboard'}catch(err){alert('ارتباط با سرور برقرار نشد. لطفاً دوباره تلاش کنید.')}finally{if(btn){btn.disabled=false;btn.textContent='ورود'}}}
...
async function loadMarket(force=false){
  ...
  try{const r=await fetch('/api/market'+(force?'?refresh=1':''),{cache:'no-store'});const j=await r.json();if(!r.ok)throw new Error(j.error||'market_unavailable');amnaMarketData=j.items||{};const keys=['gold18','coin','usd','eur','aed','btc','eth','usdt'];grid.innerHTML=keys.map(k=>marketCard(amnaMarketData[k])).join('');const t=j.fetchedAt?new Date(j.fetchedAt).toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'}):'';status.textContent=(j.stale?'آخرین قیمت معتبر نمایش داده شد — ':'')+'آخرین به‌روزرسانی: '+(t||'اکنون');if(j.warnings?.length)status.textContent+=' · برخی داده‌ها موقتاً در دسترس نبودند';}catch(e){status.textContent='قیمت‌های بازار موقتاً در دسترس نیست؛ دوباره تلاش کنید.';}}
function calcGold(){const w=+document.getElementById('goldWeight')?.value;const k=+document.getElementById('goldKarat')?.value;if(!w||w<=0)return document.getElementById('goldResult').textContent='وزن را وارد کنید.';const base=Number(amnaMarketData?.gold18?.price_toman||0);if(!base)return document.getElementById('goldResult').textContent='قیمت طلای ۱۸ هنوز دریافت نشده است.';const effectiveKarat=k===750?18:k;const value=w*base*(effectiveKarat/18);document.getElementById('goldResult').innerHTML=`ارزش تقریبی: <b>${marketFmt(value)} تومان</b><br><span class="muted">بر اساس قیمت لحظه‌ای بازار و بدون اجرت، سود فروشنده و مالیات.</span>`}
