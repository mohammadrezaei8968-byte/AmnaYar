...
async function register(e){e.preventDefault();const btn=e.submitter;if(btn){btn.disabled=true;btn.textContent='در حال ثبت‌نام...'}try{const r=await api('/api/auth/register','POST',{email:$('#email').value,username:$('#username').value,password:$('#password').value});if(r.error)return alert(r.error);if(!r.user||!r.token)return alert('ثبت‌نام انجام نشد؛ پاسخ نامعتبر از سرور دریافت شد.');localStorage.setItem(AUTH_TOKEN_KEY,r.token);closeModal();loadNavUser();location.href='/'}catch(err){alert('ارتباط با سرور برقرار نشد. لطفاً دوباره تلاش کنید.')}finally{if(btn){btn.disabled=false;btn.textContent='ثبت‌نام رایگان'}}}
async function login(e){e.preventDefault();const btn=e.submitter;if(btn){btn.disabled=true;btn.textContent='در حال ورود...'}try{const r=await api('/api/auth/login','POST',{identifier:$('#identifier').value,password:$('#password').value});if(r.error)return alert(r.error);if(!r.user||!r.token)return alert('ورود انجام نشد؛ پاسخ نامعتبر از سرور دریافت شد.');localStorage.setItem(AUTH_TOKEN_KEY,r.token);location.href='/dashboard.html'}catch(err){alert('ارتباط با سرور برقرار نشد. لطفاً دوباره تلاش کنید.')}finally{if(btn){btn.disabled=false;btn.textContent='ورود'}}}
