...
async function register(e){e.preventDefault();const btn=e.submitter;if(btn){btn.disabled=true;btn.textContent='در حال ثبت‌نام...'}try{const r=await api('/api/auth/register','POST',{email:$('#email').value,username:$('#username').value,password:$('#password').value});if(r.error)return alert(r.error);if(!r.user||!r.token)return alert('ثبت‌نام انجام نشد؛ پاسخ نامعتبر از سرور دریافت شد.');saveUserToken(r.token);location.href='/dashboard.html'}catch(err){alert('ارتباط با سرور برقرار نشد. لطفاً دوباره تلاش کنید.')}finally{if(btn){btn.disabled=false;btn.textContent='ثبت‌نام رایگان'}}}
async function login(e){e.preventDefault();const btn=e.submitter;if(btn){btn.disabled=true;btn.textContent='در حال ورود...'}try{const r=await api('/api/auth/login','POST',{identifier:$('#identifier').value,password:$('#password').value});if(r.error)return alert(r.error);if(!r.user||!r.token)return alert('ورود انجام نشد؛ پاسخ نامعتبر از سرور دریافت شد.');saveUserToken(r.token);location.href=r.user.role==='owner'?'/owner':'/dashboard'}catch(err){alert('ارتباط با سرور برقرار نشد. لطفاً دوباره تلاش کنید.')}finally{if(btn){btn.disabled=false;btn.textContent='ورود'}}}
const USER_TOKEN_KEY='amnayar_user_token';
function getUserToken(){try{return localStorage.getItem(USER_TOKEN_KEY)||''}catch{return ''}}
function saveUserToken(t){try{if(t)localStorage.setItem(USER_TOKEN_KEY,t);else localStorage.removeItem(USER_TOKEN_KEY)}catch{}}
async function api(url,method='GET',body){const token=getUserToken();const headers=body?{'Content-Type':'application/json'}:{};if(token)headers.Authorization='Bearer '+token;const r=await fetch(url,{method,credentials:'include',headers,body:body?JSON.stringify(body):undefined,cache:'no-store'});let j={};try{j=await r.json()}catch{}if(!r.ok&&!j.error)j.error=`خطای سرور (${r.status})`;return j}
...
async function logoutNav(){try{await api('/api/auth/logout','POST')}finally{saveUserToken('');location.reload()}}
loadNavUser();
