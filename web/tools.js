...
async function translateText(direction){const input=direction==='fa-en'?$('#faToEnText'):$('#enToFaText');const result=direction==='fa-en'?$('#faToEnResult'):$('#enToFaResult');const status=direction==='fa-en'?$('#faToEnStatus'):$('#enToFaStatus');const text=input.value.trim();if(!text){status.textContent='متن را وارد کنید.';return}if(text.length>5000){status.textContent='حداکثر ۵۰۰۰ نویسه مجاز است.';return}status.textContent='در حال ترجمه...';result.value='';const pair=direction==='en-fa'?'en|fa':'fa|en';try{let data={};try{const r=await fetch('/api/translate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text,direction})});if(r.ok)data=await r.json()}catch{}if(!data.translatedText){const url='https://api.mymemory.translated.net/get?q='+encodeURIComponent(text)+'&langpair='+encodeURIComponent(pair);const r=await fetch(url);if(!r.ok)throw new Error('سرویس ترجمه پاسخ نداد.');data=await r.json();data.translatedText=data?.responseData?.translatedText||data?.matches?.[0]?.translation||''}if(!data.translatedText)throw new Error('ترجمه انجام نشد.');result.value=String(data.translatedText).replace(/&amp;/g,'&').replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,'<').replace(/&gt;/g,'>');status.textContent='ترجمه آماده است.'}catch(e){status.textContent=e.message||'ترجمه انجام نشد؛ دوباره تلاش کنید.'}}
