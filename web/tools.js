...
async function imagesToPDF(){const i=$('#imagePdfFiles'),s=$('#imagePdfStatus'),fs=i?[...i.files]:[];if(!fs.length)return s.textContent='حداقل یک عکس انتخاب کنید.';if(!window.PDFLib)return s.textContent='کتابخانه PDF آماده نیست؛ چند ثانیه بعد دوباره تلاش کنید.';s.textContent='در حال ساخت PDF...';try{const doc=await PDFLib.PDFDocument.create();for(let n=0;n<fs.length;n++){const f=fs[n];const img=await new Promise((res,rej)=>{const x=new Image(),u=URL.createObjectURL(f);x.onload=()=>{URL.revokeObjectURL(u);res(x)};x.onerror=()=>{URL.revokeObjectURL(u);rej(new Error('image_load_failed'))};x.src=u});const max=1800,sc=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*sc));c.height=Math.max(1,Math.round(img.naturalHeight*sc));const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,0,0,c.width,c.height);const jpg=await new Promise((res,rej)=>c.toBlob(b=>b?res(b):rej(new Error('image_export_failed')),'image/jpeg',.9));const emb=await doc.embedJpg(await jpg.arrayBuffer());const portrait=c.height>=c.width,pw=portrait?595:842,ph=portrait?842:595,ratio=Math.min(pw/c.width,ph/c.height),dw=c.width*ratio,dh=c.height*ratio,page=doc.addPage([pw,ph]);page.drawImage(emb,{x:(pw-dw)/2,y:(ph-dh)/2,width:dw,height:dh});s.textContent='در حال پردازش عکس '+fa(n+1)+' از '+fa(fs.length)+'...';}const bytes=await doc.save({useObjectStreams:true});download(bytes,fs.length===1?'amnayar-image-to-pdf.pdf':'amnayar-images-to-pdf.pdf','application/pdf');s.textContent='PDF با '+fa(fs.length)+' عکس آماده شد و دانلود آن شروع شد.';}catch(e){console.error(e);s.textContent='تبدیل عکس به PDF انجام نشد؛ فرمت یا فایل عکس را بررسی کنید.';}}
